...
# 64 "/opt/tinyos-main-2.1.2/tos/lib/timer/Timer.nc"
static void LoraWANC$Timer0$startPeriodic(uint32_t dt);
# 104 "/opt/tinyos-main-2.1.2/tos/interfaces/SplitControl.nc"
...
#line 183
      switch (__nesc_ntoh_uint8(current_pkt->type.nxdata)) {
          ...
          if (TOS_NODE_ID == LoraWANC$server_node[sim_node()]) {
              ...
              sim_log_debug(155U, "radio_rec", "ACK sent from server %d\n", TOS_NODE_ID);
            }
          else {
...
# 64 "/opt/tinyos-main-2.1.2/tos/lib/timer/Timer.nc"
inline static void LoraWANC$Timer0$startPeriodic(uint32_t dt){
#line 64
...
#line 93
{
  if (err == SUCCESS) {
      ...
      if ((((TOS_NODE_ID == 1 || TOS_NODE_ID == 2) || TOS_NODE_ID == 3) || TOS_NODE_ID == 4) || TOS_NODE_ID == 5) {
          LoraWANC$Timer0$startPeriodic(10000);
        }
    }
  else {
...
#line 235
{
  if (&LoraWANC$packet[sim_node()] == bufPtr) {
      ...
      sim_log_debug(161U, "radio_send", "Packet sent successfully!\n");
    }
}
...
#line 116
  ...
  __nesc_hton_uint8(current_msg->type.nxdata, __nesc_ntoh_uint8(msg_to_send->type.nxdata));
  __nesc_hton_uint8(current_msg->id.nxdata, __nesc_ntoh_uint8(msg_to_send->id.nxdata));
  __nesc_hton_uint8(current_msg->sender.nxdata, __nesc_ntoh_uint8(msg_to_send->sender.nxdata));
  __nesc_hton_uint8(current_msg->content.nxdata, __nesc_ntoh_uint8(msg_to_send->content.nxdata));


  LoraWANC$actual_send(AM_BROADCAST_ADDR, &LoraWANC$packet[sim_node()]);
  sim_log_debug(153U, "radio_rec", "	msg sent at node %d\n", TOS_NODE_ID);
                               ...
                               uintptr_t* addr, size_t* size)
{
  ...
  if (!strcmp(varname, "LoraWANC$current_msg"))
  {
    *addr = (uintptr_t)&LoraWANC$current_msg[__nesc_mote];
    *size = sizeof(LoraWANC$current_msg[__nesc_mote]);
    return 0;
  }
